...
import pyarrow
import librosa
class Batch_AudioRead(FlokAlgorithmLocal):
    def run(self, inputDataSets, params):
        ...
        if "hdfs://" not in path:
            ...
            if os.path.isdir(path):
                ...
                for file_name in file_list:
                    ...
                    if os.path.isfile(file_path):
                        audio_dict[file_name]= librosa.load(file_path)
            else:
                file_name=path.split('/')[-1]
                audio_dict[file_name] = librosa.load(path)
        else:
            ip = path.split(':')[1][2:]
            tmp = path.split(':')[2]
            port = tmp[:tmp.index('/')]
            ...
            if os.path.isfile(local_tmp_path):
                audio_dict[folder_name] = librosa.load(local_tmp_path)
            else:
                ...
                for file_name in file_list:
                    if '.crc' not in file_name:
                        ...
                        if os.path.isfile(full_name):
                            audio_dict[file_name] = librosa.load(full_name)
            # 删除本地文件

...
class Batch_VideoRead(FlokAlgorithmLocal):
    def run(self, inputDataSets, params):
        ...
        if "hdfs://" not in path:
            ...
            if os.path.isdir(path):
                ...
                for file_name in file_list:
                    ...
                    if os.path.isfile(file_path):
                        video_dict[file_name] = VideoFileClip(file_path)
            else:
                file_name=path.split('/')[-1]
                video_dict[file_name] = VideoFileClip(path)
        else:
            ip = path.split(':')[1][2:]
            tmp = path.split(':')[2]
            port = tmp[:tmp.index('/')]
            ...
            if os.path.isfile(local_tmp_path):
                video_dict[folder_name] = VideoFileClip(local_tmp_path)
            else:
                ...
                for file_name in file_list:
                    if '.crc' not in file_name:
                        ...
                        if os.path.isfile(full_name):
                            video_dict[file_name] = VideoFileClip(full_name)
            # 删除本地文件

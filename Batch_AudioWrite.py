...
import time
import librosa
class Batch_AudioWrite(FlokAlgorithmLocal):
    def run(self, inputDataSets, params):
        ...
        if "hdfs://" not in path:
            ...
            for audio_name,audio in audio_dict.items():
                new_name = time.strftime('%Y_%m_%d_%H_%M_%S_') + audio_name
                file_name = os.path.join(path, new_name)
                file_name=file_name.split('.')[0]+".wav"
                librosa.output.write_wav(file_name, audio[0], audio[1])
        else:
            ip = path.split(':')[1][2:]
            tmp = path.split(':')[2]
            port = tmp[:tmp.index('/')]
